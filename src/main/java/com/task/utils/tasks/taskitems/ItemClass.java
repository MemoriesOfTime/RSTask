...
public class ItemClass implements InstanceItem {
        ...
        if (defaultString.split("@").length > 1) {
            if (defaultString.split(":").length < 2) {
                ...
                if ("item".equals(defaultString.split("@")[1])) {
                    String itemString = defaultString.split("@")[0];
                    String[] items = itemString.split(":");
                    try {
                        Item item1 = Item.fromString(items[0] + ":" + items[1]);
                        item1.setCount(Integer.parseInt(items[2]));
    ...
    public static ItemClass toItem(TaskItem item) {
        if (item.getTaskTag() == TaskItem.TaskItemTag.defaultItem) {
            ...
            String[] b = a.split(":");
            if (b.length > 1) {
                return new ItemClass(Item.fromString(b[0] + ":" + b[1]));
            } else {
                return new ItemClass(Integer.parseInt(b[0]), 0, 1);
            }
    ...
    public String toString() {
        if (item.getId() == 255) { //字符串ID物品
            return item.getNamespaceId() + ":" + item.getCount() + ":"
