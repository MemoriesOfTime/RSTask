    ...
    public void run() {
        while (true) {
            ...
            try {
                ...
                Iterator<PlayerFile> iterator = getOwner().playerFiles.values().iterator();
                while (iterator.hasNext()) {
                    PlayerFile file = iterator.next();
                    if (file.toSave()) {
                        saveCount++;
                    } else {
                        getOwner().getLogger().error("玩家 " + file.getPlayerName() + " 任务文件保存失败!");
                        errorCount++;
                    }
                    if (getOwner().getServer().getPlayerExact(file.getPlayerName()) == null) {
                        iterator.remove();
                    }
                }
                ...
                getOwner().getLogger().warning("[自动保存] 保存出现异常 " + saveTaskCount + "个任务配置保存成功" + saveCount + "个玩家任务数据保存成功 " + errorCount + " 个玩家任务数据保存失败");
            }
